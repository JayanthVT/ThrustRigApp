...
def default_init_params(df: pd.DataFrame) -> dict:
    """
    Build default initial parameters dict from the dataframe.
    - Coolant/thermal sensors: first row where value > 0 (coolant runs before motor)
    - Motor/ESC/battery sensors: first row where commanded_rpm > 0 (motor spinning)
    """
    def _first_nonzero_val(col, fmt="{:.2f}"):
        """Get value from first row where column is non-zero."""
        try:
            mask = df[col] > 0
            if not mask.any():
                return ""
            v = df.loc[mask.idxmax(), col]
            return fmt.format(float(v)) if pd.notna(v) else ""
        except Exception:
            return ""

    def _first_spin_val(col, fmt="{:.2f}"):
        """Get value from first row where commanded_rpm > 0."""
        try:
            if "Cmd_RPM" not in df.columns:
                return ""
            mask = df["Cmd_RPM"] > 0
            if not mask.any():
                return ""
    ...
    return {
        ...
        "res_temperature":     _first_nonzero_val("Motor_Inlet_Temp_C", "{:.1f}"),
        "duty_cycle":          "70% ~6lpm",
        "init_esc_temp":       _first_spin_val("ESC_Temp",           "{:.1f}"),
        "init_motor_temp":     _first_spin_val("Motor_Temp",         "{:.1f}"),
        "ambient_temp":        "",
        "esc_inlet_coolant":   _first_nonzero_val("ESC_Inlet_Temp_C",   "{:.1f}"),
        "motor_inlet_coolant": _first_nonzero_val("Motor_Inlet_Temp_C", "{:.1f}"),
        "esc_inlet_flow":      _first_nonzero_val("ESC_Flow",            "{:.2f}"),
        "motor_inlet_flow":    "-",
        "esc_inlet_pressure":  _first_nonzero_val("ESC_Pressure",        "{:.3f}"),
        "motor_inlet_pressure":"-",
        "battery_voltage":     _first_spin_val("Voltage",             "{:.2f}"),
        "battery_soc":         "-%",
        "battery_soh":         "100%",
        "fin_inlet_temp":      _first_nonzero_val("Fin_Inlet_Temp_C",   "{:.1f}"),
        "fin_outlet_temp":     _first_nonzero_val("Fin_Outlet_Temp_C",  "{:.1f}"),
        "notes":               "",
    }