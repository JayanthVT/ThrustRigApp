...
def init_db(db_path=None):
    with get_conn(db_path) as conn:
        ...
        for _col_sql in [
            ...
            "ALTER TABLE runs ADD COLUMN result_params TEXT",
        ]:
            try:
...
def update_result_params(filename, result_params, db_path=None):
    """Save manually edited result values for a run."""
    with get_conn(db_path) as conn:
        conn.execute(
            "UPDATE runs SET result_params=?, saved_at=? WHERE filename=?",
            (json.dumps(result_params),
             datetime.now().isoformat(timespec="seconds"),
             filename)
        )
        conn.commit()
