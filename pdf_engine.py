...
def build_pdf_report(data, chart_images, run_name, sections=None):
    ...
    def pv(key):
        """Paragraph with formatted value from data dict. Returns None if blank."""
        v = data.get(key, "")
        ...
        try:
            ...
            pass
        return Paragraph(str(v), sty_val)
    ...
    for grp_label, span, rows in ip_groups:
        for i, (lbl, key, unit) in enumerate(rows):
            _val = pv(key)
            _val_cell = _val if _val is not None else Paragraph("", sty_val)
            if i == 0:
                ip_data.append([
                    Paragraph(grp_label, sty_grp_bold),
                    Paragraph(lbl, sty_lbl),
                    _val_cell,
                    Paragraph(unit if _val is not None else "", sty_chk),
                ])
            else:
                ip_data.append([
